...
leafPP :: CatLabel -> PARSER Cat Cat
leafPP label []     = []
leafPP label (a:b:cs) = [(Leaf a,b:cs) | catLabel b == label ]
...
parseVPs :: [Cat] -> [([ParseTree Cat Cat],[Cat])]
parseVPs = many parsePureVP

parseMainVP :: PARSER Cat Cat
parseMainVP = mainVpRule
...
parseTense :: PARSER Cat Cat
parseTense = leafP "TENSE"

parsePureVP :: PARSER Cat Cat
parsePureVP = leafP "VP"

parseVP :: PARSER Cat Cat
parseVP = finPastVpRule <|> finPresVpRule <|> finFutVpRule <|> finPerfVpRule
          <|> auxVpRule <|> tenseVpRule
...
mainVpRule :: PARSER Cat Cat
mainVpRule = \xs ->
  [ (Branch (Cat "_" "MAINVP" [] []) (vps),zs) |
    (vps,zs)    <- parseVPs xs]
...
tenseVpRule :: PARSER Cat Cat
tenseVpRule = \xs ->
  [(Branch (Cat "_" "VP" (fs (t2c tense)) []) [tense,vp'],zs) |
  (tense,ys) <- parseTense xs,
  (vp,zs) <- vpRule ys,
  vp'    <- assignT Fut vp ]
...
prsDET = leafPS "DET" <||> claR

claR :: SPARSER Cat Cat
claR = \ us xs ->
  [ (Branch (Cat "_" "DET" fs []) [num,cla], (us++ws), zs) |
      (num,vs,ys) <- prsNUM [] xs,
      (cla,ws,zs)  <- prsCLA vs ys,
       fs         <- combine (t2c num) (t2c cla)]

prsNUM :: SPARSER Cat Cat
prsNUM = leafPS "NUM"

prsCLA :: SPARSER Cat Cat
prsCLA = leafPS "CLA"
...
prsVP = chEmptyVpR
-- <||> finPastVpR <||> finPresVpR <||> finFutVpR <||> finPerfVpR <||> auxVpR <||> tenseVpR

prsMainVP :: SPARSER Cat Cat
prsMainVP = chPastVpR <||> chFutVpR <||> chPresVpR
...
vpsR :: SPARSER Cat Cat
vpsR = \us xs ->
 [(Branch (Cat "_" "VP" [] []) (vps),ws,zs) |
             (vps,ws,zs) <- prsVPs us xs ]

manyVpsR :: SPARSER Cat Cat
manyVpsR = \us xs -> [(vp',vs,ys) | (vp,vs,ys) <- vpsR us xs,
                                   vp' <- assignT MainVP vp ]

chPastVpR :: SPARSER Cat Cat
chPastVpR = \us xs ->
 [(Branch (Cat "_" "VP" (fs(t2c par)) []) [vp',par],ws,zs) |
             (vp,vs,ys)  <- vpsR us xs,
             vp' <- assignT Past vp,
             (par,ws,zs) <- prsPAR vs ys]

chFutVpR :: SPARSER Cat Cat
chFutVpR = \us xs ->
     [ (Branch (Cat "_" "VP" (fs (t2c par)) [])
               [par,vp'], ws, zs) |
                  (par,vs,ys) <- prsPAR us xs,
                  (vp,ws,zs) <- vpsR vs ys,
                   vp'       <- assignT Fut vp ]

chPresVpR :: SPARSER Cat Cat
chPresVpR = \us xs -> [(vp',vs,ys) | (vp,vs,ys) <- vpsR us xs,
                             vp'       <- assignT Pres vp ]

chEmptyVpR :: SPARSER Cat Cat
chEmptyVpR = \us xs -> [(vp,vs,ys) | (vp,vs,ys) <- vpR us xs ]
...
prsTense :: SPARSER Cat Cat
prsTense = leafPS "TENSE" <||> pop "TENSE"
...
main = do
  putStrLn "Test"
  putStrLn $ show (parses "我 有 一 本 书")
  putStrLn $ show (parses "他 将要 看 一 本 书")
  putStrLn $ show (parses "她 看见 我们 了")
  putStrLn $ show (parses "我们 将要 写 作业")
  putStrLn $ show (parses "他们 坐 飞机 了")
  putStrLn $ show (parses "他们 去 北京 了")
  putStrLn $ show (parses "他们 坐 飞机 去 北京 了")
  putStrLn $ show (parses "张三 将要 出海")
  putStrLn $ show (parses "张三 将要 捕鱼")
  putStrLn $ show (parses "张三 将要 出海 捕鱼")
  putStrLn $ show (parses "李四 回来 了")
  putStrLn $ show (parses "李四 将要 学习")
  putStrLn $ show (parses "小明 写 诗 了")
  putStrLn $ show (parses "小明 读 书 了")
  putStrLn $ show (parses "小明 读 书 写 诗 了")
  putStrLn $ show (parses "小红 将要 回来")
  putStrLn $ show (parses "小红 将要 休息")
  putStrLn $ show (parses "小红 将要 回来  休息")
  putStrLn $ show (parses "小刚 回来 了")
  putStrLn $ show (parses "小刚 吃 饭 了")
  putStrLn $ show (parses "小刚 喝 酒 了")
  putStrLn $ show (parses "小刚 吃 饭 喝 酒 了")
  putStrLn $ show (parses "小王 看见 一 把 椅子 了")
  putStrLn $ show (parses "小王 将要 输")
  putStrLn $ show (parses "小李 赢 了")
  putStrLn $ show (parses "李白 知道 这 首 诗")
  putStrLn $ show (parses "杜甫 知道 这 本 书")
  putStrLn $ show (parses "李白 睡 了")
  putStrLn $ show (parses "她们 认识 小红")
  putStrLn $ show (parses "小李 去 美国 了")
